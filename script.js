...
async function initMultiplayer(){
  ...
  try {
    if (room && room.peers) {
      for (const pid in room.peers) {
        ...
        if (p && p.username) remoteUsernames[pid] = p.username;
      }
    }
  } catch (e) { /* ignore */ }

  // In some environments peer metadata can arrive slightly delayed; keep syncing peers->remoteUsernames
  // for a short window so labels/leaderboard update globally without requiring movement or presence edits.
  (function startPeerNameSync(timeoutMs = 8000, tickMs = 400){
    const stopAt = Date.now() + timeoutMs;
    const tick = setInterval(()=>{
      try {
        if (!room || !room.peers) {
          if (Date.now() >= stopAt) clearInterval(tick);
          return;
        }
        let foundAny = false;
        for (const pid in room.peers) {
          const p = room.peers[pid];
          if (p && p.username) {
            if (!remoteUsernames[pid]) {
              remoteUsernames[pid] = p.username;
              foundAny = true;
            }
          }
        }
        // if we've populated names for all current peers, stop early
        const peerCount = Object.keys(room.peers || {}).length;
        const cachedCount = Object.keys(remoteUsernames || {}).filter(k => room.peers && room.peers[k]).length;
        if (peerCount > 0 && cachedCount >= peerCount) {
          clearInterval(tick);
          return;
        }
        if (Date.now() >= stopAt) clearInterval(tick);
  ...
  // subscribe to room state to re-render tiles and keep reset timers synced
  room.subscribeRoomState((state) => {
    // sync map when seed changes
    const seed = state && state.seed ? state.seed : null;
    if (seed !== lastMapSeed) {
      lastMapSeed = seed;
      generateTiles();
      // ensure DOM matches authoritative wall state after regenerating tiles
      applyWallsToDOM(state.walls || {});
    }

    // if room state contains a scheduled start_reset time, use it to show the synchronized countdown
    if (state && state.start_reset_at) {
      const startAt = Number(state.start_reset_at) || null;
      if (startAt && !isNaN(startAt)) {
        // If another client is already handling the reset, ignore duplicate countdowns.
        if (resetInProgress) {
          // show the countdown UI but do not schedule another reset handler
          const now = Date.now();
          const msUntil = Math.max(0, startAt - now);
          countdownEl.style.display = 'flex';
          countdownEl.style.opacity = '1';
          countdownTimer.textContent = String(Math.ceil(msUntil / 1000));
          // do not set another interval to perform the reset; return early
          return;
        }

        // mark that this client is handling the reset sequence to avoid duplicate resets
        resetInProgress = true;

        // show countdown UI and schedule the reset to happen exactly at startAt (client-side)
        const now = Date.now();
        const msUntil = Math.max(0, startAt - now);
        countdownEl.style.display = 'flex';
        countdownEl.style.opacity = '1';
        ...
        const countdownTick = setInterval(()=>{
          ...
          if (Date.now() >= startAt) {
            ...
            const dots = generateRandomDots(seed+1);
            const axes = generateRandomAxes(seed+2);
            try { room.updateRoomState({ seed, walls, dots, axes }); } catch(e){ /* ignore */ }
            const pos = getRandomFreePosition();
            try { room.updatePresence({ row: pos.r, col: pos.c, color: getRandomColor(), value: 1 }); } catch(e){ /* ignore */ }
    ...
    // sync top-center nextReset timer if provided by room state (helps late joiners)
    if (state && state.nextResetAt) {
      const nr = Number(state.nextResetAt) || null;
      if (nr && !isNaN(nr)) {
        nextResetAt = nr;
  ...
  bindLocalControls();
  bindPresenceRequests();
  bindEvents();

  // Broadcast a short-lived request asking peers to advertise their usernames in presence.
  // Some accounts/clients may delay populating room.peers; this prompt encourages everyone to
  // write their username into presence so labels, leaderboard and other UI sync globally.
  try {
    // send an initial one immediately and then a couple of retries to help late joiners
    room.send({ type: 'request_name_sync', echo: false });
    const retries = [1200, 2600, 5200];
    retries.forEach((ms, idx) => {
      setTimeout(() => {
        try { room.send({ type: 'request_name_sync', echo: false }); } catch(e){/*ignore*/ }
      }, ms);
    });
  } catch (err) { /* ignore send failures */ }
}
